...
#        ## smart-ass way of cleaning the text from punctuations:
#        cleaner = str.maketrans(string.punctuation," "*len(string.punctuation))
#        clean_text = " ".join(story.lower().translate(cleaner).split())
        
        ## clean the text from punctuations:
        text = text.lower()
        for char in string.punctuation:
            text = text.replace(char," ")
            text = " ".join(text.split())
            
#       if re.findall("\b(" + self.phrase + ")\b", text):
        if text==self.phrase or re.findall(" "+self.phrase+" ", text) or re.findall("\A"+self.phrase+" ", text) or re.findall(" "+self.phrase+"\Z", text) :
            return True
...
def read_trigger_config(filename):
    ...
    trigger_file = open(filename, 'triggers')
    lines = []
...
def main_thread(master):
    ...
    try:
        t1 = TitleTrigger("shooting")
        t2 = DescriptionTrigger("Trump")
        t3 = DescriptionTrigger("amazon")
        t4 = AndTrigger(t2, t3)
        triggerlist = [t1, t4]
